...
class DistanceNet(torch.nn.Module, LatentValidityModel):
...
class SDFDistanceNet(torch.nn.Module, LatentValidityModel):
    ...
    def __init__(self, h_dim, z_dim, c_dim=0, n_grid=32, voxel_latent_dim=16, activation=F.relu):
        ...
        """
        super(SDFDistanceNet, self).__init__()
    ...
    def disLoss(self, y, y_hat):
        loss_estimation = F.mse_loss(y_hat, y)
        # loss_estimation = F.mse_loss(y_hat, y)
    ...
    def set_sdf(self, sdf):
        if type(sdf) is np.ndarray:
            self.sdf = torch.from_numpy(sdf).to(device=device, dtype=torch.float32)
        elif type(sdf) is torch.Tensor:
            self.sdf = sdf.to(device=device, dtype=torch.float32)
        self.sdf = self.sdf[None, :]
        self.given_sdf_latent = self.voxel_encoder(self.sdf)
