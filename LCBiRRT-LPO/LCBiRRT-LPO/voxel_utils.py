...
def getlinkPositionList(q,pc,link_name_list):
    ...
    # pc = PlanningScene(arm_names=['panda_left', 'panda_right', 'panda_top'], arm_dofs=[7,7,7])
    ...
    pc.update_joints(q)
    # pc.display(q)
    # link_name_list = [['panda_1_link0', 'panda_1_link1', 'panda_1_link2', 'panda_1_link3', 'panda_1_link4', 'panda_1_link5', 'panda_1_link6', 'panda_1_link7', 'panda_1_link8', 'panda_1_hand'],
    #                   ['panda_2_link0', 'panda_2_link1', 'panda_2_link2', 'panda_2_link3', 'panda_2_link4', 'panda_2_link5', 'panda_2_link6', 'panda_2_link7', 'panda_2_link8', 'panda_2_hand']]
...
def plotSdf(sdf):
    ...
    # f(x,y,z) = v
    # 其中x,y,z为随机数，v=x*y*z
    # x = np.linspace(0, 32, 32, dtype=int)
    # y = np.linspace(0, 32, 32, dtype=int)
    # z = np.linspace(0, 32, 32, dtype=int)
    # v = []
    # x = []
    # y = []
    # z = []
    ...
    #             y.append(j)
    #             z.append(k)
    # v = [sdf[i][i][i] for i in range(0, 32)]
    ...
    # min_v = min(v)
    # max_v = max(v)
    # color = [plt.get_cmap("YlOrRd", 100)(int(float(i - min_v) / (max_v - min_v) * 100)) for i in v]
    # # color = [plt.get_cmap("YlOrRd", 100)(float(i - min_v) / (max_v - min_v) * 100) for i in v]
    ...
    ax.grid(False)
    # im = ax.scatter(x, y, z, s=100, c=color, marker='.',alpha=0.6)
    # 2.1 增加侧边colorbar
    ...
    # fig.colorbar(im, format=matplotlib.ticker.FuncFormatter(lambda x, pos: x * (max_v - min_v) + min_v))
    # 2.2 增加坐标轴标签
    # ax.set_xlabel('x')
    # ax.set_ylabel('y')
    # ax.set_zlabel('z')
...
def getCoordinate(q, c, pc, link_name_list,constraint,exp_name):
    ...
    for i in range(len(positionList1)):
        ...
        # for j in range(len(positionList1_i)):
        #     if positionList1_i[j]<0 or positionList1_i[j]>31:
        #         abandon = True
        #         break
        # if abandon:
        #     continue
    ...
    if len(link_name_list) >1 :
        ...
        for i in range(len(positionList2)):
            ...
            # for j in range(len(positionList2_i)):
            #     if positionList2_i[j] < 0 or positionList2_i[j] > 31:
            #         abandon = True
            #         break
            # if abandon:
            #     continue
...
def getMinDis(q, c, pc, sdf, link_name_list, exp_name, positionList1_new=None, positionList2_new=None):
    ...
    #     # print(i)
    #     quat = [0, 0, 0, 1]
    #     link_name = 'link1' + str(i)
    #     pc.add_sphere(link_name, 0.06, positionList1[i], quat)
    # for i in range(len(positionList2)):
    #     # print(i)
    #     quat = [0, 0, 0, 1]
    #     link_name = 'link2' + str(i)
    #     pc.add_sphere(link_name, 0.02, positionList2[i], quat)
...
# print('------------------------------')
# vs = VisualSimulator(width=512, height=512, focal_length_x=550.0, focal_length_y=550.0, z_near=0.15, z_far=3.88)
# vs.load_scene(pc)
# vs.set_cam_and_target_pose(np.array([2.0,0, 3.0]), np.array([0.0, 0.0, 1.0]))
# np.printoptions(precision=3, suppress=True, linewidth=100, threshold=10000)
# depth = vs.generate_depth_image()
# vs.set_grid_resolution(32)
